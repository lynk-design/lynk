...
- Please **do** open your PR against the `main` branch... for now.
...
`main` - This branch reflects the latest release and powers [lynk.design](https://lynk.design/).

`next` - Coming soon! When this branch is available, you should submit all pull requests against it. It reflects what's coming in the _next_ release, which will be previewed at [next.lynk.design](https://next.lynk.design/).

## Developing

To set up a local dev environment, fork the repo on GitLab, clone it locally, and install its dependencies.

```bash
git clone https://gitlab.edgecastcdn.net/YOUR_USERNAME/lynk-design
cd lynk-design
yarn
```

Once you've cloned the repo, run the following command to spin up the Lynk dev server.
...
Lynk uses [Web Test Runner](https://modern-web.dev/guides/test-runner/getting-started/) for testing. To launch the test runner during development, open a terminal and launch the dev server.
...
```bash
yarn test -- --group <nameOfTestFile>
```
...
<lynk-alert type="info" open>This section can be a lot to digest in one sitting, so don't feel like you need to take it all in right now. Most contributors will be better off skimming this section and reviewing the relevant content as needed.<lynk-alert>

### Accessibility

Lynk is built with accessibility in mind. Creating generic components that are fully accessible to users with varying capabilities across a multitude of circumstances is a daunting challenge. Oftentimes, the solution to an a11y problem is not written in black and white and, therefore, we may not get it right the first time around. There are, however, guidelines we can follow in our effort to make Lynk an accessible foundation from which applications and websites can be built.
...
Maintaining good documentation can be a painstaking task, but poor documentation leads to frustration and makes the project less appealing to users. Fortunately, writing documentation for Lynk is fast and easy!

Most of Lynk's technical documentation is generated with JSDoc comments and TypeScript metadata from the source code. Every property, method, event, etc. is documented this way. In-code comments encourage contributors to keep the documentation up to date as changes occur so the docs are less likely to become stale. Refer to an existing component to see how JSDoc comments are used in Lynk.
...
All components have a host element, which is a reference to the `<lynk-*>` element itself. Make sure to always set the host element's `display` property to the appropriate value depending on your needs, as the default is `inline` per the custom element spec.
...
See the source of dialog, or drawer for examples.

### Dynamic Slot Names and Expand/Collapse Icons

A pattern has been established in `<lynk-accordion>` for expand/collapse icons that animate on open/close. In short, create two slots called `expand-icon` and `collapse-icon` and render them both in the DOM, using CSS to show/hide only one based on the current open state. Avoid conditionally rendering them. Also avoid using dynamic slot names, such as `<slot name=${open ? 'open' : 'closed'}>`, because Firefox will not animate them.

There should be a container element immediately surrounding both slots. The container should be animated with CSS by default and it should have a part so the user can override the animation or disable it. Please refer to the source and documentation for `<lynk-accordion>` for details.
...
```html
<slot name="icon">
  <lynk-icon part="close-icon"></lynk-icon>
</slot>
```
...
Components must only emit custom events, and all custom events must start with `on:`, `before:`, or `after:` as a namespace. For compatibility with frameworks that utilize DOM templates, custom events must have lowercase, kebab-style names. For example, use `on:my-change` instead of `on:myChange`.
...
When change events are emitted by Lynk components, they should be named `on:change` and they should only be emitted as a result of user input. Programmatic changes, such as setting `el.value = '…'` _should not_ result in a change event being emitted. This is consistent with how native form controls work.
...
Then use the following syntax for comments so they appear in the generated docs. Do not use the `--lynk-` prefix, as that is reserved for design tokens that live in the global scope.
 ...
 */
@customElement('lynk-example')
export default class LynkExample {
  // ...
}
```
...
render() {
  return html`
    <div part="base">
      <lynk-icon part="icon" exportparts="base:icon__base" ...></lynk-icon>
    </div>
  `;
}
```
...
Avoid inlining SVG icons inside of templates. If a component requires an icon, make sure `<lynk-icon>` is a dependency of the component and use the [system library](/components/icon#customizing-the-system-library):

```html
<lynk-icon library="system" name="..."></lynk-icon>
```
