...
describe('<l-include>', () => {
  ...
  it('should load content and emit l-load', async () => {
    ...
    const el = await fixture<SlInclude>(html` <l-include src="/found"></l-include> `);
    const loadHandler = sinon.spy();

    el.addEventListener('l-load', loadHandler);
  ...
  it('should emit l-error when content cannot be loaded', async () => {
    sinon.stub(window, 'fetch').resolves({
      ...stubbedFetchResponse,
      ok: false,
      status: 404,
    ...
    const el = await fixture<SlInclude>(html` <l-include src="/not-found"></l-include> `);
    const loadHandler = sinon.spy();

    el.addEventListener('l-error', loadHandler);
    await waitUntil(() => loadHandler.calledOnce);
