 ...
 * @status stable
 *
 * @event on:reposition - Emitted when the popup is repositioned. This event can fire a lot, so avoid putting expensive
 ...
 *  attribute is used.
 * @cssproperty [--arrow-color=var(--lynk-color-neutral-0)] - The color of the arrow.
 ...
 *  available when using `auto-size`.
 */
@customElement('lynk-popup')
export default class LynkPopup extends LynkElement {
  ...
  @query('.lynk-popup') public popup: HTMLElement;
  @query('.lynk-popup__arrow') private arrowEl: HTMLElement;
  ...
  render() {
    return html`
      ...
      <div
        ...
        class=${classMap({
          'lynk-popup': true,
          'lynk-popup--active': this.active,
          'lynk-popup--fixed': this.strategy === 'fixed',
          'lynk-popup--has-arrow': this.arrow
        })}
      >
        <slot></slot>
        ${this.arrow ? html`<div part="arrow" class="lynk-popup__arrow" role="presentation"></div>` : ''}
      </div>
    `;
  }
}
