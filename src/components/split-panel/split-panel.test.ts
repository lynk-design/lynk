...
import type LynkSplitPanel from './split-panel';

const DIVIDER_WIDTH_IN_PX = 4;

const getPanel = (splitPanel: LynkSplitPanel, testid: string): HTMLElement => {
  const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
  expect(startPanel).not.to.be.null;
  return startPanel!;
};

const getPanelWidth = (splitPanel: LynkSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { width } = panel.getBoundingClientRect();
  return width;
};

const getPanelHeight = (splitPanel: LynkSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { height } = panel.getBoundingClientRect();
  return height;
};

const getDivider = (splitPanel: LynkSplitPanel): Element => {
...
describe('<lynk-split-panel>', () => {
  ...
  it('should render a component', async () => {
    const splitPanel = await fixture(html` <lynk-split-panel></lynk-split-panel> `);
