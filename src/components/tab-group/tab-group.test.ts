...
import type LynkTabGroup from './tab-group';
...
const waitForScrollButtonsToBeRendered = async (tabGroup: LynkTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('lynk-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: LynkTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<LynkTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('lynk-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: LynkTabPanel) => element.hasAttribute('active'));
...
const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<LynkTab> => {
  const generalHeader = queryByTestId<LynkTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<lynk-tab-group>', () => {
  it('renders', async () => {
    const tabGroup = await fixture<LynkTabGroup>(html`
      <lynk-tab-group>
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<LynkTabGroup>(html`
      <lynk-tab-group>
        <lynk-tab slot="nav" panel="general" data-testid="general-tab-header">General</lynk-tab>
        <lynk-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</lynk-tab>
        <lynk-tab-panel name="general">This is the general tab panel.</lynk-tab-panel>
  ...
  describe('scrolling behavior', () => {
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<LynkTabGroup>(html`<lynk-tab-group> ${generateTabs(30)} </lynk-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<LynkTabGroup>(html`<lynk-tab-group> ${generateTabs(2)} </lynk-tab-group>`);
    ...
    it('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<LynkTabGroup>(html`<lynk-tab-group> ${generateTabs(50)} </lynk-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('lynk-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: LynkTabGroup, action: () => Promise<void>): Promise<void> => {
      ...
      const customHeader = queryByTestId<LynkTab>(tabGroup, 'custom-header');
      expect(customHeader).not.to.have.attribute('active');

      const showEventPromise = oneEvent(tabGroup, 'on:tab-show') as Promise<CustomEvent>;
      await action();
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'on:tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'on:tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<LynkTabGroup>(html`
        <lynk-tab-group>
          ...
          <lynk-tab slot="nav" panel="custom" data-testid="custom-header">Custom</lynk-tab>
          <lynk-tab-panel name="general">This is the general tab panel.</lynk-tab-panel>
          <lynk-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</lynk-tab-panel>
        </lynk-tab-group>
      `);
