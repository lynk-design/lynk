...
describe('<l-avatar>', () => {
  ...
  describe('when provided an image and label parameter', () => {
    ...
    it('should pass accessibility tests', async () => {
       ...
       * aria-label which dictates what "l-avatar" is. This also implies that label text will
  ...
  describe('when passed a <span>, on slot "icon"', () => {
    ...
    it('should accept as an assigned child in the shadow root', () => {
      const slot = el.shadowRoot!.querySelector<HTMLLynkotElement>('slot[name=icon]')!;
